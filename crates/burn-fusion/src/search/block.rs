...
pub struct Block<O> {
    ...
    ordering: Vec<usize>,
    /// For each builder, the index into `operations` of the operation that closed it, once one
    /// has. What [optimize](Self::optimize) settles when nothing fuses from the block's head.
...
impl<O: NumOperations> Block<O> {
    ...
    /// Optimize the block.
    ///
    /// `fresh` are the builders as a new search starts them: they tell, after an unfusable
    /// head, which operations no builder would even start on.
    pub fn optimize(mut self, fresh: &[Box<dyn OperationFuser<O>>]) -> BlockOptimization<O> {
        match find_best_optimization_index(&mut self.builders) {
            ...
            BestOptimization::NotFound => {
                // Nothing fuses from the head of this block. The operations after the one that
                // closed the last builder are only here because they depend on that head; no
                // builder ever saw them on their own. Settle the head unfused, along with
                // whatever follows it that no builder would start on, and leave the rest to
                // the next search — as a hole or as the next segment — where a fresh builder
                // gets to fuse it.
                let settled = self.operations_settled_once_every_builder_closed();
                let settled = self.operations_settled_while_no_builder_would_start(settled, fresh);
    ...
    fn operations_settled_while_no_builder_would_start(
        &self,
        mut settled: usize,
        fresh: &[Box<dyn OperationFuser<O>>],
    ) -> usize {
        while let Some(operation) = self.operations.get(settled) {
            let wanted = fresh.iter().any(|builder| {
    ...
    /// How many operations, from the head, it takes to close every builder. A builder still
    /// open at the end counts the whole block: there is no point after which it stopped
    /// wanting more.
