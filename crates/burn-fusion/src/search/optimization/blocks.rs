...
impl<O: NumOperations> BlocksOptimizer<O> {
    ...
    ///
    /// `fresh` are the builders as a new search starts them; see [Block::optimize].
    pub fn optimize(mut self, fresh: &[Box<dyn OperationFuser<O>>]) -> BlocksOptimizerResult<O> {
        ...
        for block in blocks {
            let mut block_opt = block.optimize(fresh);
