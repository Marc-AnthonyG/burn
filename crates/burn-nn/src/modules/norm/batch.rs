...
mod tests_1d {
    ...
    /// The statistics the training op hands back join the running-statistics
    /// update on the same device. They come back untracked and without a
    /// checkpointing strategy of their own, so that update merges with
    /// whatever strategy the device trains under.
    #[test]
