...
/// scatters in parallel. The serial kernel above runs one unit per output row
/// and walks the scatter axis, which leaves the device nearly idle whenever the
/// values outnumber the output rows by a lot — a few thousand rows receiving
/// millions of updates is the shape a histogram or a counting bank produces.
/// Only for `Add`, and only on types the device adds atomically.
#[cube(launch_unchecked, address_type = "dynamic")]
