...
use crate::{
    kernel::{utils::address_type, utils::decompose_linear},
    ops::{max_vector_size, numeric::empty_device_dtype, permute, reshape},
    tensor::CubeTensor,
};
...
/// `input` is NHWC with the channels contiguous; `weight` is `[kh, kw, c]` with the channels
/// contiguous. Both may be pitched on their other dimensions.
...
fn depthwise_conv2d_kernel<E: Numeric, N: Size>(
    ...
    let in_h = input.shape(1) as u32;
    let in_w = input.shape(2) as u32;
    let kernel_h = weight.shape(0) as u32;
    let kernel_w = weight.shape(1) as u32;
    ...
    for ky in 0..kernel_h {
        ...
        if has_padding {
            row_in_bounds = in_y >= 0 && (in_y as u32) < in_h;
        }
        ...
        for kx in 0..kernel_w {
            ...
            if has_padding {
                in_bounds &= in_x >= 0 && (in_x as u32) < in_w;
            }
            ...
            if in_bounds {
                let in_pos = in_batch_offs
                    ...
                    + c as usize;
                let weight_pos = ky as usize * weight.stride(0)
                    + kx as usize * weight.stride(1)
                    + c as usize * weight.stride(2);
...
pub(crate) fn conv_depthwise<const N: usize>(
    ...
    // `[c, kh, kw, 1]` to `[kh, kw, c]`, so a tap's weights for neighbouring channels are
    // neighbours in memory. The unit dimension has to go: the vector size is taken along the
    // last dimension, and a trailing dimension of one would pin it to one.
    let weight = reshape(
        permute(weight, &[1, 2, 0, 3]),
        Shape::new([kernel_shape[0], kernel_shape[1], channels]),
    );
